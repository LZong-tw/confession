...
while True:
    while not reply_queue.empty() or not stop_queue.empty():
        if stop_queue.get():
            ...
            responseTexts = random.choice(default)
        else:
            responseTexts = reply_queue.get()
        print("答：" + responseTexts)
        voice_name = "cmn-TW-Wavenet-A"
        # language_code = "-".join(voice_name.split("-")[:2])
        # language_code = "fr-CA"
        language_code = "zh-TW"
        text_input = tts.SynthesisInput(text=responseTexts)
